...
Adaptive sampling requires a storage backend to store the observed traffic data and computed probabilities. At the moment only Cassandra is supported as sampling backend. We are seeking help in implementing support for other backends ([tracking issue](https://github.com/jaegertracing/jaeger/issues/3305)).
