...
A sample configuration for Jaeger with ClickHouse backend is available in the Jaeger repository: [config-clickhouse.yaml](https://github.com/jaegertracing/jaeger/blob/v2.21.0/cmd/jaeger/config-clickhouse.yaml). In the future the configuration documentation will be auto-generated from the schema. Meanwhile, please refer to [config.go](https://github.com/jaegertracing/jaeger/blob/v2.21.0/internal/storage/v2/clickhouse/config.go) as the authoritative source.

## Schema

All SQL definitions are available in the [sql/ directory](https://github.com/jaegertracing/jaeger/tree/v2.21.0/internal/storage/v2/clickhouse/sql) in the Jaeger repository.
