...
To use a remote storage as Jaeger storage backend, use `grpc` as the storage type and specify the remote gRPC server address. For more information, please refer to [jaeger/internal/storage/v1/grpc](https://github.com/jaegertracing/jaeger/tree/v2.5.0/internal/storage/v1/grpc).

Example config for remote storage [can be found here](https://github.com/jaegertracing/jaeger/blob/v2.5.0/cmd/jaeger/config-remote-storage.yaml).
