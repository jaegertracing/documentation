...
A sample configuration for Jaeger with Cassandra backend is available in the Jaeger repository: [config-cassandra.yaml](https://github.com/jaegertracing/jaeger/blob/v2.18.0/cmd/jaeger/config-cassandra.yaml). In the future the configuration documentation will be auto-generated from the schema. Meanwhile, please refer to [config.go](https://github.com/jaegertracing/jaeger/blob/v2.18.0/pkg/cassandra/config/config.go#L21) as the authoritative source.
...
If `schema.create` is set to `false`, the schema must be initialized manually. There is a script in the [jaeger](https://github.com/jaegertracing/jaeger/blob/v2.18.0/internal/storage/v1/cassandra/schema/create.sh) repository that generates the initialization instruction that can be executed using Cassandra's interactive shell [cqlsh][cqlsh]:
...
See [this README](https://github.com/jaegertracing/jaeger/blob/v2.18.0/internal/storage/v1/cassandra/schema/README.md) for more details on Cassandra schema management.
...
* ScyllaDB [can be used](https://github.com/jaegertracing/jaeger/blob/v2.18.0/internal/storage/v1/scylladb/README.md) as a drop-in replacement for Cassandra since it uses the same data model and query language.
